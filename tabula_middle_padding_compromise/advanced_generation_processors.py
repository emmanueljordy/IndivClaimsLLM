"""Advanced Generation Processors

Implements dataset-agnostic column profiling and value-only generation constraints
to fix gen_limit computation and eliminate empty cells, overlong values, and 
contamination tokens.

Key Features:
1. ColumnProfiler: Build-time analysis of value-only statistics
2. ColumnProfile: Type detection and constraint specification
3. FlattenedTrie: Exact multi-token categorical enforcement
4. ColumnValueProcessor: Stateful logits processor with fallback strategies
5. Comprehensive instrumentation and debugging

This system is fully generic - no column names or hard-coded categories.
"""
...
class TrieCursor:
    ...
    def __init__(self, trie: 'FlattenedTrie'):
        self.trie = trie
        self.step = 0
        self.prefix_tokens = []
...
class FlattenedTrie:
    ...
    def get_allowed_tokens(self, step: int, prefix_tokens: List[int]) -> Set[int]:
        ...
        for full_tokens in self._get_all_training_sequences():
            if (len(full_tokens) >= step and 
    ...
    def _get_all_training_sequences(self) -> List[List[int]]:
        """Get all training sequences (reconstructed from trie)."""
        # This is a simple implementation - in practice you'd store the original sequences
        # For now, we'll track them during construction
        if not hasattr(self, '_training_sequences'):
            self._training_sequences = []
        return self._training_sequences
...
class ColumnProfiler:
    ...
    def profile_column(self, column_name: str, values: List[str], 
        ...
        if column_type == ColumnType.SMALL_CATEGORICAL:
            ...
            profile.trie._training_sequences = tokenized_values  # Store for reference
...
class ColumnValueProcessor(LogitsProcessor):
    ...
    This processor combines all the techniques:
    1. Value-only gen_limit enforcement
    2. Type-specific masking (trie, numeric, soft)
    3. Contamination prevention (name tokens, specials)
    4. Fallback strategies for mask collapse
    5. Hard closing when limits reached
    """
    ...
    def enter_column(self, col_idx: int, running_ids: List[List[int]]):
        """Enter new column: reset state and append space to context sequences.
        
        This method:
        1. Resets all state counters to 0 (important!)
        2. Appends the space token to each sequence in the context
        3. Does NOT call update_state_with_token for the space
        4. Does NOT increment any counters for the space
        
        Args:
            col_idx: Index of column being entered
            running_ids: List of token sequences to modify
        """
        # 1. Set column state - RESET to 0
        self.column_state.current_column_idx = col_idx
        self.column_state.value_step = 0
        self.column_state.emitted_nonspace_count = 0
        self.column_state.value_tokens = []
        
        # 2. Reset trie state if applicable
        self._reset_trie_state(col_idx)
        
        # 3. Force append space token to each sequence (A1 approach)
        space_id = self.special_ids.get('space_id')
        if space_id is not None:
            for seq in running_ids:
                seq.append(space_id)
        
        # 4. CRITICAL: Do NOT call update_state_with_token(space_id)
        # Do NOT increment emitted_nonspace_count or value_step for the space
        # The space is just context; the counters should remain at 0 until a real value token
    
    def advance_to_column(self, column_name: str):
        """Advance to specific column (for segment-wise generation) - DEPRECATED.
        
        Use enter_column() instead for proper space handling.
        """
        try:
            ...
            pass  # Column not found, keep current state
    ...
    def should_force_close(self) -> bool:
        ...
        # For small categoricals, force close if trie indicates completion
        if (profile.column_type == ColumnType.SMALL_CATEGORICAL and 
            profile.trie and 
            profile.trie.can_close_here(self.column_state.value_step)):
            # Only close if we have at least one non-space token
    ...
    def __call__(self, input_ids: torch.Tensor, scores: torch.Tensor) -> torch.Tensor:
        ...
        # This runs ALWAYS, not just in fallback
        # Since space is already in context, we don't need to allow space_id here
        if self.column_state.emitted_nonspace_count == 0:
        ...
        # 2) Always ban other special tokens during value generation (we inject them manually)
        ...
        # Note: Separator blocking is handled by the hard guard above
        # Note: Space is already in context (appended before tensor conversion), so we don't block space_id
        #       The first generated token should be a value token, not a space
    ...
    def _apply_fallback_strategy(self, scores: torch.Tensor, profile: ColumnProfile):
        ...
        # (This is redundant with the main __call__ guard, but keeping for extra safety in fallback)
        if self.column_state.emitted_nonspace_count == 0:
            colsep_id = self.special_ids.get('colsep_id')
            rowsep_id = self.special_ids.get('rowsep_id')
            if colsep_id is not None and colsep_id < scores.size(-1):
...
def print_profiling_summary(profiles: Dict[str, ColumnProfile], tokenizer=None):
    ...
    for col_name, profile in profiles.items():
        ...
        if profile.column_type == ColumnType.SMALL_CATEGORICAL and profile.trie:
            print(f"  Trie: max_depth={profile.trie.max_depth}, can_close_at={sorted(profile.trie.can_close_at)[:5]}")
