#!/usr/bin/env python3
"""Optimized OpenELM-450M training script with QLoRA and advanced optimizations."""
...
def main():
    parser = argparse.ArgumentParser(description="Train OpenELM-450M with QLoRA optimizations")
    ...
    parser.add_argument("--num_epochs", type=int, help="Number of additional epochs for continue training (overrides default 50)")
    ...
    # Tokenizer setup - OpenELM uses LLaMA-2 tokenizer
    print("Setting up tokenizer...")
    # OpenELM uses LLaMA tokenizer which requires add_bos_token=True
    tokenizer = AutoTokenizer.from_pretrained(
        "hf-internal-testing/llama-tokenizer",  # Public LLaMA tokenizer (not gated)
        add_bos_token=True,                     # Required for OpenELM
        use_fast=True                           # Fastest implementation
    )
    ...
    # Dataset-specific MAX_TOKENS - optimized for OpenELM-450M
    # Use 95th percentile + buffer for complete record generation
    recommended_tokens = int(np.percentile(lengths, 95) * 1.4)  # 40% buffer for medium model
    ...
    GENERATION_MAX_LENGTH = max(MAX_TOKENS * 2.5, 500)  # Medium multiplier for 450M model
    print(f"Using GENERATION_MAX_LENGTH: {GENERATION_MAX_LENGTH}")

    # QLoRA Configuration optimized for OpenELM-450M (GPU only)
    if device == "cuda":
        qlora_quantization_config = {
            "load_in_4bit": True,
            "bnb_4bit_use_double_quant": True,
            "bnb_4bit_quant_type": "nf4",
            "bnb_4bit_compute_dtype": torch.float16  # Use float16 for OpenELM efficiency
        }
    else:
    ...
    # LoRA configuration optimized for OpenELM-450M architecture
    qlora_lora_config = {
        "r": 48,                    # Medium rank for 450M model
        "lora_alpha": 96,           # Proportional alpha
        "target_modules": ["qkv_proj", "out_proj", "proj_1", "proj_2"],  # OpenELM-specific layer names
        "lora_dropout": 0.08,       # Medium dropout for 450M model
        "bias": "none",
        "use_rslora": True,
    ...
    if args.fresh_training:
        print("=" * 60)
        print("FRESH TRAINING: Starting OpenELM-450M with QLoRA (4-bit + LoRA)")
        print("=" * 60)
    ...
    if args.continue_training and pathlib.Path(final_model_path).exists():
        print("=" * 60)
        print("CONTINUE TRAINING: Loading existing OpenELM-450M QLoRA model")
        print("=" * 60)
        ...
        print(f"Existing OpenELM-450M model loaded in {load_time:.2f}s")
        ...
        if args.num_epochs is not None:
            ...
            print(f"Continuing training for 50 additional epochs")
            tabula_qlora.epochs = 50
            
        # Update batch size
        tabula_qlora.batch_size = 32
        
        # Continue training
        print("Continuing OpenELM-450M training...")
        start_time = time.time()
        try:
        ...
        print(f"OpenELM-450M continue training completed in {training_time:.2f}s")
    ...
    if not needs_training and not args.continue_training:
        print("=" * 60)
        print("LOADING: Existing OpenELM-450M QLoRA model")
        print("=" * 60)
        print(f"Found existing model at: {final_model_path}")
        ...
        if args.fresh_training:
            ...
            print("=" * 60)
            print("INITIALIZING: OpenELM-450M with QLoRA (4-bit + LoRA)")
            print("=" * 60)
        ...
        start_time = time.time()
        tabula_qlora = Tabula(
            llm='apple/OpenELM-450M',
            tokenizer=tokenizer,
            experiment_dir=str(logging_path),
            epochs=args.num_epochs if args.num_epochs else 35,  # Medium epochs for 450M model
            batch_size=32,  # Medium batch size for 450M model
            ...
            categorical_temperature=1.25 if use_diversity else 1.0,  # Slightly higher for OpenELM
            small_categorical_temperature=1.55 if use_diversity else 1.0,
            max_seq_length=MAX_TOKENS,
            gradient_accumulation_steps=8,  # Medium accumulation for 450M
            learning_rate=1.8e-4,  # Optimized learning rate for OpenELM
            warmup_steps=180,      # Medium warmup steps
            weight_decay=0.01,
            lr_scheduler_type="cosine",
        )

        init_time = time.time() - start_time
        print(f"OpenELM-450M QLoRA initialization time: {init_time:.2f}s")
        ...
        # Training
        print("Training OpenELM-450M with QLoRA...")
        start_time = time.time()
        
        trainer_qlora = tabula_qlora.fit(df_llm, debug_training=True, use_qlora=args.use_qlora)
        
        training_time = time.time() - start_time
        print(f"OpenELM-450M QLoRA training completed in {training_time:.2f}s")
    ...
    synthetic_path = output_path / "synthetic_data_openelm.csv"
    try:
