#!/usr/bin/env python3
"""Optimized SmolLM2-360M training script with QLoRA and advanced optimizations."""
...
def main():
    parser = argparse.ArgumentParser(description="Train SmolLM2-360M with QLoRA optimizations")
    ...
    parser.add_argument("--num_epochs", type=int, help="Number of additional epochs for continue training (overrides default 35)")
    ...
    tokenizer = AutoTokenizer.from_pretrained("HuggingFaceTB/SmolLM2-360M", trust_remote_code=True)
    ...
    # Dataset-specific MAX_TOKENS - optimized for small model
    # Use 95th percentile + buffer for complete record generation
    recommended_tokens = int(np.percentile(lengths, 95) * 1.3)  # 30% buffer for small model
    MAX_TOKENS = max(128, recommended_tokens)  # Lower minimum for small model
    ...
    GENERATION_MAX_LENGTH = max(MAX_TOKENS * 2, 400)  # Smaller multiplier for 360M model
    print(f"Using GENERATION_MAX_LENGTH: {GENERATION_MAX_LENGTH}")

    # QLoRA Configuration optimized for SmolLM2-360M (GPU only)
    if device == "cuda":
        qlora_quantization_config = {
            "load_in_4bit": True,
            "bnb_4bit_use_double_quant": True,
            "bnb_4bit_quant_type": "nf4",
            "bnb_4bit_compute_dtype": torch.float16  # Use float16 for small model efficiency
        }
    else:
    ...
    # LoRA configuration optimized for SmolLM2-360M (smaller model, lower ranks)
    # Note: target_modules will be auto-detected by Tabula for SmolLM2
    qlora_lora_config = {
        "r": 32,                    # Lower rank for small 360M model
        "lora_alpha": 64,           # Proportional alpha
        # target_modules will be auto-detected by Tabula for SmolLM2 architecture
        "lora_dropout": 0.1,        # Slightly higher dropout for small model
        "bias": "none",
        "use_rslora": True,
    ...
    if args.fresh_training:
        print("=" * 60)
        print("FRESH TRAINING: Starting SmolLM2-360M with QLoRA (4-bit + LoRA)")
        print("=" * 60)
    ...
    if args.continue_training:
        if pathlib.Path(final_model_path).exists():
            print("=" * 60)
            print("CONTINUE TRAINING: Loading existing SmolLM2-360M QLoRA model")
            print("=" * 60)
            ...
            print(f"Existing SmolLM2-360M model loaded in {load_time:.2f}s")
            ...
            if args.num_epochs is not None:
                ...
                print(f"Continuing training for 35 additional epochs")
                tabula_qlora.epochs = 35
                
            # Update batch size
            tabula_qlora.batch_size = 32
            
            # Continue training
            print("Continuing SmolLM2-360M training...")
            start_time = time.time()
            try:
            ...
            print(f"SmolLM2-360M continue training completed in {training_time:.2f}s")
        ...
        print("=" * 60)
        print("LOADING: Existing SmolLM2-360M QLoRA model")
        print("=" * 60)
        print(f"Found existing model at: {final_model_path}")
        ...
        if args.fresh_training:
            ...
            print("=" * 60)
            print("INITIALIZING: SmolLM2-360M with QLoRA (4-bit + LoRA)")
            print("=" * 60)
        ...
        tabula_qlora = Tabula(
            ...
            epochs=args.num_epochs if args.num_epochs else 35,  # More epochs for small model
            batch_size=32,  # Larger batch size for small 360M model
            categorical_columns=categorical_columns,
            use_lora=True,
            lora_config=qlora_lora_config,
            quantization_config=qlora_quantization_config,
            # Diversity enhancement settings for better categorical distributions
            use_diversity_enhancement=use_diversity,
            categorical_temperature=1.3 if use_diversity else 1.0,  # Higher for SmolLM2 to improve diversity
            small_categorical_temperature=1.6 if use_diversity else 1.0,  # Even higher for small categoricals
            max_seq_length=MAX_TOKENS,
            gradient_accumulation_steps=8,  # Less accumulation for small model
            learning_rate=2e-4,  # Higher learning rate for small model
        ...
        print(f"SmolLM2-360M QLoRA initialization time: {init_time:.2f}s")
        ...
        # Training
        training_method = "QLoRA" if args.use_qlora else "full fine-tuning"
        print(f"Training SmolLM2-360M with {training_method}...")
        start_time = time.time()
        
        trainer_qlora = tabula_qlora.fit(df_llm, debug_training=True, use_qlora=args.use_qlora)
        
        training_time = time.time() - start_time
        print(f"SmolLM2-360M QLoRA training completed in {training_time:.2f}s")
    ...
    print("\nGenerating synthetic samples with SmolLM2-360M QLoRA and optimizations...")
    print(f"Target samples: {args.num_samples}")
    print(f"Max length: {GENERATION_MAX_LENGTH} tokens")
    print(f"Temperature: 0.8")  # Higher temperature for small model diversity
    print(f"Generation batch size: 128")  # Large batch size for small model
    start_time = time.time()
    
    synthetic_data = tabula_qlora.sample(
        n_samples=args.num_samples,
        temperature=0.9,            # Higher temperature for small model
        max_length=GENERATION_MAX_LENGTH,
        device=device,
        use_flash_attention=True,   # Enable flash attention for small model
        use_kv_cache=True,          # Enable KV cache for speed
        batch_size_generation=128,  # Large batch size for 360M model
    ...
    synthetic_path = output_path / "synthetic_data_smollm2.csv"
    try:
