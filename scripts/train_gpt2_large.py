#!/usr/bin/env python3
"""Optimized GPT-2 Large training script with QLoRA and advanced optimizations."""
...
def main():
    parser = argparse.ArgumentParser(description="Train GPT-2 Large with QLoRA optimizations")
    ...
    parser.add_argument("--num_epochs", type=int, help="Number of additional epochs for continue training (overrides default 30)")
    ...
    if args.hf_token:
        ...
        try:
            ...
            print("⚠️ huggingface_hub not available, relying on environment variable")
    ...
    if args.dataset == "bi":
        ...
        # Set categorical columns for BI after cleaning
        ...
        # Set categorical columns for EH after cleaning
    ...
    print(f"Number of elements in first row: {len(df_llm.iloc[0])}")
    print(f"Column names: {df_llm.columns.tolist()[:10]}...")  # Show first 10 columns
    ...
    # Dataset-specific MAX_TOKENS for training
    # Use 95th percentile + buffer for training context
    ...
    # Generation max_length should be much higher to ensure complete records
    GENERATION_MAX_LENGTH = max(MAX_TOKENS * 3, 600)  # At least 3x training tokens or 600
    ...
    print(f"Using MAX_TOKENS for training: {MAX_TOKENS}")
    print(f"Using GENERATION_MAX_LENGTH: {GENERATION_MAX_LENGTH}")

    # QLoRA Configuration optimized for GPT-2 Large (GPU only)
    if device == "cuda":
        qlora_quantization_config = {
            "load_in_4bit": True,
            "bnb_4bit_use_double_quant": True,
            "bnb_4bit_quant_type": "nf4",
            "bnb_4bit_compute_dtype": torch.float16  # GPT-2 optimized
        }
    else:
    ...
    if args.fresh_training:
        print("🆕 FRESH TRAINING: Skipping model registry check")
    ...
    if args.fresh_training:
        print("=" * 60)
        print("FRESH TRAINING: Starting GPT-2 Large with QLoRA (4-bit + LoRA)")
        print("=" * 60)
        ...
        tabula_qlora = Tabula(
            ...
            epochs=args.num_epochs if args.num_epochs else 30,
            batch_size=32,  # Optimized for GPT-2 Large
            ...
            categorical_temperature=1.15 if use_diversity else 1.0,  # Lower for GPT-2 Large (more capable model)
            small_categorical_temperature=1.4 if use_diversity else 1.0,
        ...
        print(f"GPT-2 Large QLoRA initialization time: {init_time:.2f}s")
        ...
        # Fresh training
        print("Training GPT-2 Large with QLoRA (fresh training)...")
        start_time = time.time()
        
        trainer_qlora = tabula_qlora.fit(df_llm, debug_training=True, use_qlora=args.use_qlora)
        
        training_time = time.time() - start_time
        print(f"GPT-2 Large QLoRA fresh training completed in {training_time:.2f}s")
        
        # Save and register the trained model
        print(f"Saving and registering fresh trained model...")
        ...
        print("Fresh trained model saved and registered successfully")
        
    elif args.continue_training:
        if pathlib.Path(final_model_path).exists():
            print("=" * 60)
            print("CONTINUE TRAINING: Loading existing GPT-2 Large QLoRA model")
            print("=" * 60)
            ...
            print(f"Existing GPT-2 Large model loaded in {load_time:.2f}s")
            ...
            if args.num_epochs is not None:
                ...
                print(f"Continuing training for 30 additional epochs")
                tabula_qlora.epochs = 30
                
            # Update batch size
            tabula_qlora.batch_size = 32
            
            # Continue training
            print("Continuing GPT-2 Large training...")
            start_time = time.time()
            try:
                trainer_qlora = tabula_qlora.fit(df_llm, debug_training=True, use_qlora=args.use_qlora)
            except Exception as e:
                ...
                raise
            training_time = time.time() - start_time
            print(f"GPT-2 Large continue training completed in {training_time:.2f}s")
        ...
        print("=" * 60)
        print("LOADING: Existing GPT-2 Large QLoRA model")
        print("=" * 60)
        print(f"Found existing model at: {final_model_path}")
        ...
        print("=" * 60)
        print("INITIALIZING: GPT-2 Large with QLoRA (4-bit + LoRA)")
        print("=" * 60)
        ...
        # Training
        print("Training GPT-2 Large with QLoRA...")
        start_time = time.time()
        
        trainer_qlora = tabula_qlora.fit(df_llm, debug_training=True, use_qlora=args.use_qlora)  # Use full dataset for better learning
        
        training_time = time.time() - start_time
        print(f"GPT-2 Large QLoRA training completed in {training_time:.2f}s")
    ...
    print("\nGenerating synthetic samples with optimizations...")
    print(f"Target samples: {args.num_samples}")
    print(f"Generation max length: {GENERATION_MAX_LENGTH} tokens")
    print(f"Temperature: 0.5")
    print(f"Generation batch size: 8")
    
    # Debug: Show expected format vs actual format
    print("\nTRAINING FORMAT VERIFICATION:")
    sample_training_text = row_to_text(df_llm.iloc[0])
    print(f"Expected format: {sample_training_text[:150]}...")
    print(f"Format contains {sample_training_text.count(' | ')} pipe separators")
    print(f"Expected columns: {df_llm.columns.tolist()[:5]}...")
    
    start_time = time.time()
    
    synthetic_data = tabula_qlora.sample(
        n_samples=args.num_samples,
        temperature=0.9,  # Reduced for more focused generation
        max_length=GENERATION_MAX_LENGTH,
        device=device,
        use_flash_attention=False,
        use_kv_cache=True,
        batch_size_generation=64,  # Reduced for better quality
    ...
    if len(synthetic_data) == 0:
        print("No valid samples generated! Check generation parameters.")
        print("Debug info:")
        print(f"   - Model trained on {len(df_llm)} samples")
        print(f"   - Token length range: {np.min(lengths)}-{np.max(lengths)}")
        print(f"   - Training MAX_TOKENS: {MAX_TOKENS}")
        print(f"   - Generation MAX_LENGTH: {GENERATION_MAX_LENGTH}")
        print("Potential causes:")
        print("   - Text-to-tabular parsing failing")
        print("   - Column format mismatch")
        print("   - All generated records invalid")
        sys.exit(1)
    elif len(synthetic_data) < args.num_samples * 0.1:  # Less than 10% success
        print("Very low success rate. Consider:")
    ...
    synthetic_path = output_path / "synthetic_data_gpt2_large.csv"
    try:
